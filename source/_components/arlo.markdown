...
Finish its configuration by visiting the [Arlo sensor page](/components/arlo/) or [Arlo camera page](/components/arlo/) or [Arlo control panel page](/components/arlo/). Arlo also has a service call `arlo.update` that can be manually called to force an update prior to the regular scheduled interval.
